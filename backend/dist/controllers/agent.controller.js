...
class AgentController {
    static async codeReview(req, res) {
        try {
            const { code, language } = req.body;
            if (!code) {
                return res.status(400).json({ error: 'Code is required' });
            }
            const agent = agent_factory_1.AgentFactory.getAgent('code_review');
    ...
    static async bugFix(req, res) {
        try {
            const { error, context } = req.body;
            if (!error) {
                return res.status(400).json({ error: 'Error description is required' });
            }
            const agent = agent_factory_1.AgentFactory.getAgent('bug_fix');
    ...
    static async sqlGenerator(req, res) {
        try {
            const { query, context } = req.body;
            if (!query) {
                return res.status(400).json({ error: 'Query description is required' });
            }
            const agent = agent_factory_1.AgentFactory.getAgent('sql_generator');
